...
renderProducts();
        ...
        function toggleWishlist(productId) {
            ...
            if (idx !== -1) wishlist.splice(idx, 1);
                ...
                const p = allProducts.find(a => a.id === productId);
        ...
        function renderWishlist() {
            ...
            const gridContainer = wishlistGrid.querySelector('div');
            if (wishlist.length === 0) {
                gridContainer.innerHTML = `<p class="col-span-full text-center text-gray-600">Your wishlist is empty.</p>`;
                return;
            }
            gridContainer.innerHTML = wishlist.map(generateProductCard).join('');
            bind3DAndButtons(gridContainer);
        }

        function renderShoppingBag() {
            if (!bagGrid) return;
            const gridContainer = bagGrid.querySelector('div');
        ...
        function findProductById(productId) {
            // First try to find from dynamic products
            let product = allProducts.find(p => p.id === productId);
            if (product) return product;

            // Then try to find from element's data attributes
        ...
        /************* Utilities to render all category grids *************/
        function renderAllCategoryGrids() {
            renderProducts(cordSetProducts, cordGrid);
            renderProducts(poloProducts, poloGrid);
            renderProducts(hoodieProducts, hoodieGrid);
            renderProducts(racingProducts, racingGrid);
            renderProducts(trackProducts, trackGrid);
            renderProducts(jerseyProducts, jerseyGrid);
        }

        /************* Page rendering (navigation) *************/
        function showOnly(sectionIds = []) {
            const allSections = ['explore-styles', 'CORD', 'POLOS', 'HOODIES', 'RACING', 'TRACK', 'JERSEYS', 'WISHLIST', 'BAG', 'video-section'];
        ...
        function renderPage(page, pushState = true) {
            ...
            const sectionsToDisplay = {
                'home': ['explore-styles', 'video-section', 'CORD', 'POLOS', 'HOODIES', 'RACING', 'TRACK', 'JERSEYS'],
                'wishlist': ['WISHLIST'],
                'bag': ['BAG'],
                'women': ['explore-styles', 'video-section', 'CORD', 'POLOS', 'HOODIES'],
                'men': ['explore-styles', 'video-section', 'RACING', 'TRACK', 'JERSEYS']
            };
